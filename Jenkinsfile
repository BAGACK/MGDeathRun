pipeline {
   ...
   stages {
      ...
      stage ('Build') {
          ...
          post {
              failure {
                  mail to: 'mep_eisen@web.de', subject: 'build of DeathRun failed', body: 'build of DeathRun failed'
              }
          }
      }
      ...
      stage ('Upload') {
        steps {
            ...
            sh "/srv/hudson/upload_game.sh ${env.BUILDTYPE} target 1 DeathRun MGDeathRun-Plugin"
      	}
      }
      ...
      stage ('Deploy') {
          ...
          post {
              failure {
                  mail to: 'mep_eisen@web.de', subject: 'deploy of DeathRun failed', body: 'deploy of DeathRun failed'
              }
          }
      }
   }
